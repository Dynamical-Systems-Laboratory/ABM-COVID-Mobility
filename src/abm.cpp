...
void ABM::create_workplaces(const std::string fname)
{
	...
	for (auto& work : file){
		...
		Workplace temp_work(std::stoi(work.at(0)), 
			...
			infection_parameters.at("workplace transmission rate"), work.at(3));
		// Store 
...
void ABM::create_leisure_locations(const std::string fname)
{
	...
	for (auto& lsr : file) {
		...
		Leisure temp_lsr(std::stoi(lsr.at(0)), 
			...
			infection_parameters.at("severity correction"), 
			infection_parameters.at("leisure locations transmission rate"),	"None");
		// Store 
...
void ABM::initialize_vac_and_reopening()
{
	...
	double work_tr_rate = infection_parameters.at("workplace transmission rate") * 
	...
	for (auto& workplace : workplaces){
		if (workplace.outside_town()) {
			...
			workplace.change_transmission_rate(work_tr_rate);
		}
	}
...
void ABM::set_outside_workplace_transmission()
{
	for (auto& workplace : workplaces) {
		if (workplace.outside_town()) {
			workplace.set_outside_lambda(infection_parameters.at("out-of-town workplace transmission"));
		}
	}
}
