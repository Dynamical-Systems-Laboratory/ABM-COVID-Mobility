				...
				ABM& abm, int infected_0)
{
	...
	// counted into sum_dpos so they should be excluded (that's the sump_dpos == sum_dinf)
	// This is different now though - since the epidemic does not die out, and some of the
	// infected are still not confirmed positive, this equality now turns inequality
	...
	// Again, not all infection cases are done so it should be <=
